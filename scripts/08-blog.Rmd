...
#putting them together for analysis
...
#pulling out best ones for state
models <- models %>% 
...
#this creates simulation all in one huge chunk
ec_dist_df <- ec_polls_df %>% 
  ...
  mutate(democrat_d = map(ec_poll_vep, ~ filter(., party == "democrat")),
    ...
    mutate(sd_r = map_dbl(republican_r, ~ sd(.$prob_poll))) %>% 
  mutate(sd_d = map_dbl(democrat_d, ~ sd(.$prob_poll))) %>% 
  ...
  unnest() %>%
  mutate(r_prob = map(.x = prob_Rvote_2020, .y = sd_r, ~rnorm(10000, mean = .x, sd = .y))) %>% 
  mutate(d_prob = map(.x = prob_Dvote_2020, .y = sd_d, ~rnorm(10000, mean = .x, sd = .y))) %>% 
  select(state, VEP, r_prob, d_prob) %>% 
  unnest(cols = c(r_prob, d_prob)) %>% 
  mutate(VEP = as.integer(VEP)) %>% 
  drop_na() %>% 
  mutate(sim_Dvotes_2020 = map(.x = d_prob , .y = VEP, ~rbinom(1, size = .y, prob = .x))) %>% 
  mutate(VEP = as.integer(VEP)) %>% 
  mutate(sim_Rvotes_2020 = map(.x = r_prob , .y = VEP, ~rbinom(1, size = .y, prob = .x)))

#ec votes
ec_votes <- read_xlsx("../data/ec_votes.xlsx")

#get simulations
#lose some due to negative votes—can't happen
ec_dist_clean_df <- ec_dist_df %>% 
  mutate(sim_Dvotes_2020 = as.numeric(sim_Dvotes_2020),
         sim_Rvotes_2020 = as.numeric(sim_Rvotes_2020)) %>% 
  mutate(winner = if_else(sim_Rvotes_2020 > sim_Dvotes_2020, "Trump", "Biden")) %>% 
  mutate(n = seq(1:10000)) %>% 
  left_join(ec_votes) %>% 
  mutate(biden_votes = if_else(winner == "Biden", votes, 0),
         trump_votes = if_else(winner == "Trump", votes, 0)) %>% 
  group_by(n) %>% 
  summarize(trump_total = sum(trump_votes),
            biden_total = sum(biden_votes)) %>% 
  drop_na() %>% 
  pivot_longer(trump_total:biden_total) %>% 
  mutate(name = if_else(name == "trump_total", "Trump", "Biden"))


#work for chances winning
ec_dist_clean_df %>% 
  filter(name == "Trump",
         value == 269) %>% 
  nrow()

#histograms for predicitons of electoral college 
ec_dist_clean_df %>% 
  ggplot(aes(x = value,
             fill = name)) +
  geom_histogram(color = 'white',
                 ...
                 bins = 20) +
  scale_fill_manual(values = c("blue2", "red2")) +
  labs(title = "Prediction of Electoral College",
       subtitle = "Using historic polling trends and current weighted polls",
       x = "Electoral College Total",
       y = "Count",
       caption = "10,000 binomial process simulations",
       fill = "Candidate") +
    scale_x_continuous(breaks = c(100, 150, 200, 250, 300, 350, 400)) +
  theme_minimal() +
  theme(plot.title = element_text(face = "bold", size = 20), 
        axis.title.x = element_text(size = 13),
        axis.title.y = element_text(size = 13)) +
  ...
  theme(plot.caption = element_text(size = 10)) +
    geom_text(aes(x = 110,
            y = 1300, 
            label = "Biden Win: 96.2%\nTrump Win: 3.5%\nTie: 0.3%"))

#save plot
ggsave("../figures/ec_total.png", height = 5, width = 7)
```
